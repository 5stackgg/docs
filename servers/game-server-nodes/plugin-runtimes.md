...
The two frameworks are **not** interchangeable at the plugin level. A CounterStrikeSharp plugin will not load under SwiftlyS2, and vice versa. See [Custom Plugins](/servers/game-server-nodes/custom-plugins) for the per-framework directory layouts.
